...
public class DeleteCommandParser {
	...
	public static final Pattern LIST_SPECIFIED_DELETE_COMMAND_FORMAT = Pattern.compile("(\\p{Alpha}+)(\\s)(\\p{Digit}+)");
    ...
    public Command parse(String args) throws ParseException {
        ...
        try {
        	...
        	if (matcherNoListSpecified.matches()) {
                ...
                if (listType.equals(ACTIVE_TASKS_STRING)) return new DeleteCommand(index);
                if (listType.equals(COMPLETE_TASKS_STRING)) return new DeleteCompleteCommand(index);
        	}
        	
            else throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, DeleteCommand.MESSAGE_USAGE));
