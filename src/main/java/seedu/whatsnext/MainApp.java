    ...
    private Model initModelManager(Storage storage, UserPrefs userPrefs) {
        ...
        try {
            ...
            if (!taskManagerOptional.isPresent()) {
                logger.info("Data file not found. Will be starting with a sample WhatsNext");
            }
            initialData = taskManagerOptional.orElseGet(SampleDataUtil::getSampleAddressBook);
        } catch (DataConversionException e) {
            logger.warning("Data file not in the correct format. Will be starting with an empty WhatsNext");
            initialData = new TaskManager();
        } catch (IOException e) {
            logger.warning("Problem while reading from the file. Will be starting with an empty WhatsNext");
            initialData = new TaskManager();
        }
