...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `TaskListPanel`, `StatusBarFooter`, `CompletedTaskListPanel` etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
|`* *` |user |restore a completed task |move a completed task from completed task list to active task list 
|`* *` |user |set reminders for tasks |remember to complete it before the deadline or display overdue tasks
|`* *` |user |show me the command history |view all commands that I used before
