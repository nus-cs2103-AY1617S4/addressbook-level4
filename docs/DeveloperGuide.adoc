...
|`* * *` |user |mark a task as being complete/ incomplete |keep track of which tasks I still need to do
...
|`* * *` |user |set a task to repeat in the future |easily manage recurring tasks

|`* *` |user |filter tasks based on tags |organise my tasks into categories
|`* *` |user |set reminders for tasks |remember to complete it before the deadline
|`* *` |user |display my task in calendar |view my tasks in a chronological format
|`* *` |user |use shortcuts to perform certain functions |navigate the program with ease and speed
|`* *` |user |colour code my tasks according to tag |view my task categories more easily
|`* *` |user |add priorities to the task |determine which task to complete first
|`* *` |user |block out more than one time slot for a task |reserve time slots if the task details are not yet final
|`* *` |user |push back all my tasks for the day by a certain amount of time |edit my tasks if unexpected events disrupt my day
|`* *` |user |view my tasks within a specified duration |manage my tasks within different time frames
